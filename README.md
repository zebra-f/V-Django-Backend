...
*(Meilisearch is not essential for running this application as a standalone REST API, If you wish to disable it and omit the rest of this paragraph edit the source code: `core/settings.py` and set the `disabled` key in `MEILISEARCH` dictionary to `True`.  
Note that a model responsible for synchronizing data between the database and the search engine will still be created and updated.)*
...
(Ensure to add `meili_data/` volume to the `.gitignore`, alternatively execute this command in a different directory suited for a storage).  
    ...
    $ # command that creates `speeds`, the Meilisearch index
