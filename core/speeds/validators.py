...
class TagsValidator:
    def __call__(self, tags: list[str], max_length: int = 4):
        ...
        if not isinstance(tags, list):
            error_messages.append("The data type provided is incorrect; it should be a list.")
            valid = False
        if isinstance(tags, list) and len(tags) > max_length:
            error_messages.append(f"Maximum limit exceeded. The list can have a maximum of {max_length} items.")
            valid = False

        if isinstance(tags, list):
            for tag in tags:
                if not isinstance(tag, str):
                    error_messages.append("The data's item type provided is incorrect; it should be a string.")
                    valid = False
                if isinstance(tag, str) and len(tag) > 20:
                    error_messages.append("Tag length exceeds the maximum limit. Each tag can have a maximum of 20 characters.")
                    valid = False

                pattern = r"^[a-zA-Z0-9'-]+$"
                if isinstance(tag, str) and not re.fullmatch(pattern, tag):
                    error_messages.append("Tags must consist of letters (both uppercase and lowercase), numbers, and the following symbols (excluding the next dot): ' - .")
                    valid = False
